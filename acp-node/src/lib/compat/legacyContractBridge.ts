// Ported verbatim from @Virtual-Protocol/acp-cli:src/lib/compat/legacyContractBridge.ts
...
export class LegacyContractBridge extends BaseAcpContractClient {
  ...
  async handleOperation(
    ...
    const txnHash = (Array.isArray(result) ? result[0] : result) as Address
    if (!txnHash) {
      throw new Error("sendCalls returned no transaction hash")
    }
