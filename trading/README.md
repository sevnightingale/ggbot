...
The ggbots trading engine supports two modes:
...
- **Live Trading**: Production non-custodial perpetual futures trading via Hyperliquid (self-custody via API wallet)

Both modes share the same decision engine, configuration system, and dashboard interface for seamless switching between simulation and live trading. Routing is driven by `trading_mode` (`'paper'` or `'hyperliquid'`).

## Recent Updates

**Paper Trading Engine 2.0:** Migrated from direct PostgreSQL to Supabase integration with full dashboard connectivity and real-time data display.
...
### Live Trading Flow
```
Decision Module → Hyperliquid Trading Service → Hyperliquid DEX → live_trades DB
                          ↓                          ↓                  ↓
                  Apply Config Defaults      Real Execution      Batch ID (UUID)
                          ↓                          ↓                  ↓
                  Size + Leverage            Position Open       Decision Linkage
                          ↓                          ↓                  ↓
              Dashboard SSE Stream ← Enrich with Hyperliquid Info API → Real-time UI
```
...
# Execute trade intent (same format as paper)
...
**live_trades Table** (`provider='hyperliquid'`):
```sql
...
3. **Config Loading Fixed**: Configs with `config_type: 'scheduled_trading'` load correctly
